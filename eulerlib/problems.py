...
def p51(min_family_size=8, limit=1_000_000):
    """Smallest prime that is part of a family of `min_family_size`
    primes, where the family is generated by replacing a fixed set
    of identical digits within the number with each of 0-9 (skipping
    replacements that would put a leading zero). Searches primes in
    increasing order, and for each one, every subset of positions
    that share a common digit, replacing that subset digit-by-digit
    and counting how many results are prime."""
    ...
    for p in range(2, limit + 1):
        ...
        for r in range(1, n):
            for combo in combinations(range(n), r):
                first_digit = digits[combo[0]]
                if not all(digits[i] == first_digit for i in combo):
                    continue
                family = []
                for d in "0123456789":
                    if combo[0] == 0 and d == "0":
                        continue
